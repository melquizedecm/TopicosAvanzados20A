...
class Users{
	...
	function create($datos){
		$sql="INSERT INTO notas (name_client, total) VALUES ('".$datos[0]."', '".$datos[1]."')";
	...
	function readId($id_user){
		$sql="SELECT * FROM notas WHERE id_nota='".$id_user."'";
		$tabla=$this->link->query($sql);
	...
	function update($datos){
		$sql="UPDATE notas SET name_client='".$datos[1]."', total='".$datos[2]."' WHERE id_nota='".$datos[0]."' ";
		$respuesta=$this->link->query($sql);
		if (!$respuesta) {
			echo $this->link->error;
		}else{
