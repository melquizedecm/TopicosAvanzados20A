...
class Users{
	...
	function create($datos){
		$sql="INSERT INTO products (name, price) VALUES ('".$datos[0]."', '".$datos[1]."')";
	...
	function readId($id_user){
		$sql="SELECT * FROM products WHERE id_product='".$id_user."'";
		$tabla=$this->link->query($sql);
	...
	function update($datos){
		$sql="UPDATE products SET name='".$datos[1]."', price='".$datos[2]."' WHERE id_product='".$datos[0]."' ";
		$respuesta=$this->link->query($sql);
		if (!$respuesta) {
			echo $this->link->error;
		}else{
